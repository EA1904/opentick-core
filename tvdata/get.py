...
from tvdata.config import DB_PATH, LAKE_PATTERN, LAKE_ROOT
...
def sql(query: str) -> pd.DataFrame:
    ...
    symbol_match = re.search(
        r"symbol\s*(?:=\s*'([^']+)'|IN\s*\(\s*'([^']+)'\s*\))", query, re.IGNORECASE
    )
        ...
        if tf:
            ...
            if sym and os.path.exists(db_path):
                try:
                    with sqlite3.connect(db_path) as db_conn:
                        ...
                        cursor.execute(
                            "SELECT asset_class FROM data_catalog WHERE symbol = ? LIMIT 1",
                            (sym.upper(),),
                        )
                        row = cursor.fetchone()
                        if row:
            ...
            if asset_class:
                ...
                if sym:
                    ...
                    scan_pattern = os.path.join(
                        LAKE_ROOT, "ohlcv", "*", f"timeframe={tf}", "**", "*.parquet"
                    ).replace(os.sep, "/")
    ...
    with _db_lock:
        ...
        if scan_pattern != _last_scan_pattern:
            if os.path.exists(test_dir) and any(os.scandir(test_dir)):
                conn.execute(
                    f"CREATE OR REPLACE VIEW ohlcv AS SELECT * FROM parquet_scan('{scan_pattern}', hive_partitioning=true)"
                )
            else:
...
def get_ohlcv(
    ...
    # Build query clauses
    where_clauses = [f"timeframe = '{timeframe}'", f"symbol IN ({symbol_list_str})"]
    if start:
