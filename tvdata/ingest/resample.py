...
def resample_symbol_data(symbol: str):
    ...
    pattern_1m = os.path.join(LAKE_ROOT, "asset_class=stocks", "timeframe=1m", f"symbol={symbol}", "**", "*.parquet")
    ...
    try:
        df_1m = db_conn.execute(f"SELECT * FROM parquet_scan('{pattern_1m}', hive_partitioning=true)").df()
    ...
    for tf, freq in timeframes.items():
        ...
        tf_dir = os.path.join(LAKE_ROOT, "asset_class=stocks", f"timeframe={tf}", f"symbol={symbol}")
        ...
        """, (
            symbol, tf, 'stocks', min_date, max_date, rows_count, nulls_pct, quality_score, 
            resampled['source'].iloc[0] or 'resampled_1m', datetime.now().isoformat()
        ))
