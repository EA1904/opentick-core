...
def ingest_dolt_corporate_actions(stocks_repo_path: str):
    ...
    """
    print(
        f"Ingesting corporate actions (splits & dividends) from Dolt: {stocks_repo_path}"
    )

    # 1. Fetch splits
    split_query = (
        "SELECT act_symbol as symbol, ex_date as date, to_factor, for_factor FROM split"
    )
    ...
    if len(df_splits) > 0:
        ...
        df_splits = pd.DataFrame(
            columns=["symbol", "date", "action_type", "split_factor", "dividend_amount"]
        )
    ...
    df_actions.to_parquet(
        os.path.join(dest_dir, "corporate_actions.parquet"), index=False
    )
...
def ingest_dolt_options(options_repo_path: str):
    ...
    if len(df_vol) > 0:
        df_vol = df_vol.rename(columns={"date": "timestamp", "act_symbol": "symbol"})
    ...
    try:
        symbols_df = query_dolt(
            options_repo_path, "SELECT DISTINCT act_symbol FROM option_chain"
        )
    ...
    for symbol in symbols:
        ...
        try:
            ...
            if len(df_symbol) > 0:
                df_symbol = df_symbol.rename(
                    columns={"date": "timestamp", "act_symbol": "symbol"}
                )
                df_symbol["timestamp"] = pd.to_datetime(df_symbol["timestamp"])

                group_path = os.path.join(dest_dir, f"{symbol}.parquet")
                df_symbol = df_symbol.sort_values(
                    by=["timestamp", "expiration", "strike"]
                )
