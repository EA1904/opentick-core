...
def parse_filename_metadata(filename: str):
    ...
    match_underscore = re.match(
        r"^([A-Za-z0-9\-]+)_(M1|M5|M15|M30|H1|H4|D1|W1|MN1)$", base, re.IGNORECASE
    )
    if match_underscore:
...
def ingest_single_metatrader5_file(
    file_path: str,
    symbol: str = None,
    timeframe: str = None,
    ...
    print(
        f"Ingesting MT5 file: {filename} (symbol={symbol}, timeframe={timeframe}, asset_class={asset_class}, tz={source_tz})"
    )
    ...
    if has_header:
        ...
        is_combined = (
            " " in first_part
            or ("/" in first_part and ":" in first_part)
            or ("-" in first_part and ":" in first_part)
            or ("." in first_part and ":" in first_part)
        )
    ...
    df.columns = [
        str(c).upper().replace("<", "").replace(">", "").strip() for c in df.columns
    ]
    ...
    print(
        f"Successfully ingested MT5 file to Parquet and registered. Symbol: {symbol}, Rows: {len(standardized)}"
    )
...
def ingest_metatrader5(
    ...
    if os.path.isdir(path):
        ...
        print(
            f"Batch MT5 ingestion completed. Successfully processed {len(results)} files."
        )
    else:
