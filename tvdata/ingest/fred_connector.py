...
from tvdata.config import WORKSPACE_ROOT, LAKE_ROOT
...
def ingest_fred_series(series_id: str, 
                        symbol: str = None, 
    ...
    if not api_key or api_key == "your_fred_api_key_here":
        ...
        print(f"Warning: Valid FRED_API_KEY not found in environment. Skipping FRED series {series_id}.")
        return {}
    ...
    print(f"Ingesting FRED series: {series_id} (symbol={symbol}) from {start_date} to {end_date}...")
    ...
    try:
        ...
        nulls_pct = (df['value'].isnull().sum() / len(df)) * 100.0 if len(df) > 0 else 0.0
        ...
        print(f"Successfully ingested FRED series {series_id} into {symbol.lower()}.parquet. Rows: {len(df)}")
        return {
            'symbol': symbol,
            'timeframe': 'D1',
            'rows': len(df),
