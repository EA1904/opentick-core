...
def ingest_alpaca(
    ...
    if start_dt < limit_date:
        print(
            f"Warning: Alpaca free tier is limited to last 2 years. Truncating start date from {start_date} to {limit_date.strftime('%Y-%m-%d')}"
        )
        start_dt = limit_date

    if start_dt >= end_dt:
        print(
            f"No new dates to query for {symbol} ({timeframe}) after Alpaca limitation check."
        )
        return {}
    ...
    print(
        f"Downloading Alpaca data for {symbol} ({interval}) from {start_iso} to {end_iso}..."
    )
    ...
    print(
        f"Successfully ingested Alpaca data for {symbol} ({timeframe}). Rows: {len(standardized)}"
    )
    return {
        "symbol": symbol.upper(),
        "timeframe": timeframe,
