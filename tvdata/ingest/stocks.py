...
from tvdata.catalog import init_db, register_dataset, register_metadata
    ...
    for chunk in pd.read_csv(csv_path, chunksize=chunk_size):
        ...
        for symbol, group in standardized.groupby("symbol"):
            ...
            if symbol not in stats:
                ...
                s = stats[symbol]
                s["start"] = min(s["start"], min_ts)
                s["end"] = max(s["end"], max_ts)
                s["rows"] += rows
                s["nulls"] += nulls
...
def ingest_archive_1m(folder_path: str):
    ...
    for filename in csv_files:
        ...
        try:
            ...
            max_ts = standardized["timestamp"].max()
            start_date = (
                min_ts.strftime("%Y-%m-%d %H:%M:%S") if pd.notnull(min_ts) else "N/A"
            )
            end_date = (
                max_ts.strftime("%Y-%m-%d %H:%M:%S") if pd.notnull(max_ts) else "N/A"
            )
