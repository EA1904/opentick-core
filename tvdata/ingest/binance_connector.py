...
def fetch_binance(symbol: str, timeframe: str, start: str, end: str) -> pd.DataFrame:
    ...
    print(
        f"Downloading Binance data for {ccxt_symbol} ({interval}) from {start} to {end}..."
    )
    ...
    while since < end_ms:
        try:
            ohlcv = exchange.fetch_ohlcv(
                symbol=ccxt_symbol, timeframe=interval, since=since, limit=1000
            )
            if not ohlcv:
                break
...
def ingest_binance(symbol: str, timeframe: str, start_date: str, end_date: str) -> dict:
    """
    ...
    print(
        f"Successfully ingested Binance data for {storage_symbol} ({timeframe}). Rows: {len(standardized)}"
    )
    return {
        "symbol": storage_symbol,
        "timeframe": timeframe,
