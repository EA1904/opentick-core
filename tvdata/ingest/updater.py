...
def update_data_lake(symbols: list = None, asset_classes: list = None):
    ...
    if len(datasets) == 0:
        print("No datasets registered in data_catalog yet. Please run initial ingestion first.")
        return
    ...
    if asset_classes:
        ...
        datasets = datasets[datasets['asset_class'].str.lower().isin(asset_classes_lower)]
    ...
    for idx, row in datasets.iterrows():
        ...
        try:
            ...
            if start_update >= now:
                print(f"Skipping {symbol} ({timeframe}): already up-to-date (end_date: {end_date_str})")
                count_skipped += 1
                continue
    ...
    print("\n==================================================")
    print(f"UPDATE PROCESS COMPLETE. {count_success} series updated, {count_skipped} skipped/up-to-date.")
    print("==================================================")
