...
def main():
    ...
    try:
        ...
        print(
            "Erreur de lecture de la table historical_sp500_tickers. Avez-vous exécuté l'ingestion de la liste ?",
            e,
        )
        return
    finally:
    ...
    try:
        ...
        df_all = db_conn.execute(
            f"SELECT * FROM parquet_scan('{old_pattern}', hive_partitioning=true)"
        ).df()
        ...
        # Filtre
        mask = (df_all["asset_class"] != "stocks") | (
            df_all["symbol"].isin(sp500_tickers)
        )
