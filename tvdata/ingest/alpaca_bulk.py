...
def run_alpaca_bulk(timeframe: str = "15m"):
    ...
    try:
        catalog_df = pd.read_sql_query(
            f"SELECT symbol, end_date FROM data_catalog WHERE timeframe = '{timeframe}'", conn
        )
    ...
    print(f"Starting bulk Alpaca ingestion for {len(symbols)} stocks (timeframe: {timeframe})...")
    ...
    for idx, symbol in enumerate(symbols):
        ...
        print(f"[{idx+1}/{len(symbols)}] Ingesting {symbol} from {start_date_str} to {end_date_str}...")
