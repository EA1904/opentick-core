...
def fetch_sec_json(url: str, user_agent: str) -> dict:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print(
                f"SEC API returned 403 Forbidden for URL: {url}. Please configure a-valid User-Agent."
            )
            return {}
        else:
...
def extract_fact(
    facts: dict, taxonomy: str, keys: list, form_types=["10-Q", "10-K"]
) -> dict:
...
def ingest_sec_financials(symbol: str) -> dict:
    ...
    if (
        not user_agent
        or "admin@tradovera.local" in user_agent
        and os.environ.get("FRED_API_KEY") == "your_fred_api_key_here"
    ):
    ...
    debt_data = extract_fact(
        facts, taxonomy, ["LongTermDebtNoncurrent", "LongTermDebt"]
    )
    ocf_data = extract_fact(
        facts, taxonomy, ["NetCashProvidedByUsedInOperatingActivities"]
    )
    capex_data = extract_fact(
        facts, taxonomy, ["PaymentsToAcquirePropertyPlantAndEquipment"]
    )
        ...
        rows.append(
            {
                ...
                "fiscal_period": "Q"
                if "-Q" in d
                else "FY",  # Placeholder: parse from date if needed
    ...
    df_new["free_cash_flow"] = df_new["operating_cf"] - df_new["capex"].abs().fillna(
        0.0
    )
    ...
    if os.path.exists(dest_file):
        try:
            ...
            print(
                f"Successfully merged SEC financials for {symbol}. Total rows: {len(df_combined)}"
            )
            return {"symbol": symbol, "rows": len(df_combined), "status": "merged"}
        except Exception as e:
            print(
                f"Error merging with existing financials for {symbol}: {e}. Writing new file."
            )
