...
from tvdata.config import DB_PATH, LAKE_ROOT, PROGRESS_FILE, BLOOMBERG_DIR
...
def main():
        ...
        try:
            for _, row in df_meta.iterrows():
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, 'US')
                """, (row['Symbol'].strip(), row['Longname'], row['Sector'], row['Industry'], row['Marketcap']))
            conn.commit()
    ...
    funds_path = os.path.join(BLOOMBERG_DIR, "processed", "sp500_fundamentals_monthly.csv")
    if os.path.exists(funds_path):
        ...
        # Clean ticker
        df_funds['symbol'] = df_funds['Ticker'].str.replace(' US Equity', '', regex=False).str.strip()
        ...
        df_funds.to_parquet(os.path.join(out_funds_dir, "fundamentals.parquet"), compression='snappy')
        print("Fondamentaux mensuels enregistrés.")
    else:
        print("Fichier fondamentaux non trouvé.")

    # 3. Ingestion de la Volatilité Daily Bloomberg
    vol_path = os.path.join(BLOOMBERG_DIR, "processed", "sp500_price_volatility_daily.csv")
    if os.path.exists(vol_path):
        ...
        # Clean ticker
        df_vol['symbol'] = df_vol['Ticker'].str.replace(' US Equity', '', regex=False).str.strip()
        df_vol['DATE'] = pd.to_datetime(df_vol['DATE'])
        
        # Save to Parquet
        out_vol_dir = os.path.join(LAKE_ROOT, "bloomberg")
        df_vol.to_parquet(os.path.join(out_vol_dir, "volatility.parquet"), compression='snappy')
    ...
    if not os.path.exists(prices_path):
        ...
        write_progress(100, 100, "Erreur: Fichier des prix daily Bloomberg introuvable.", "Bloomberg")
        return
        
    print("Chargement des prix daily Bloomberg (ceci peut prendre quelques secondes)...")
    ...
    for idx, sym in enumerate(unique_tickers, 1):
        ...
        if idx % 10 == 0 or idx == total_tickers:
            print(status_msg)
            write_progress(15 + int((idx / total_tickers) * 80), 100, status_msg, "Bloomberg")
            
        # Get Bloomberg data for this symbol
        bloom_sym_df = df_prices[df_prices['Ticker'] == sym].copy()
        
        # Map columns to standard format
        bloom_sym_df = bloom_sym_df.rename(columns={
            'Date': 'timestamp',
            'Ticker': 'symbol',
            'Open': 'open',
            'High': 'high',
        ...
        # Reorder columns
        cols = ['timestamp', 'symbol', 'open', 'high', 'low', 'close', 'volume', 'adj_factor', 'year', 'asset_class', 'timeframe']
        bloom_sym_df = bloom_sym_df[cols]
        
        # Load existing data from lake
        symbol_folder = os.path.join(LAKE_ROOT, "ohlcv", "asset_class=stocks", "timeframe=D1", f"symbol={sym}")
        existing_df = pd.DataFrame()
        if os.path.exists(symbol_folder):
            try:
                existing_df = db_conn.execute(f"SELECT * FROM parquet_scan('{symbol_folder.replace(os.sep, '/')}/*.parquet')").df()
            except Exception as e:
        ...
        if len(existing_df) > 0:
            ...
            outside_range = existing_df[(existing_df['timestamp'] < bloomberg_min) | (existing_df['timestamp'] > bloomberg_max)]
    ...
    write_progress(100, 100, f"Ingestion Bloomberg terminée avec succès. {total_tickers} symboles traités.", "Bloomberg")
