...
from tvdata.catalog import init_db, recalculate_catalog_entry, register_dataset
from tvdata.ingest.normalizer import rename_and_standardize
from tvdata.ingest.stocks import write_parquet_hive
from tvdata.quality import compute_quality_score, scan_nulls
...
def fetch_yfinance(symbol: str, timeframe: str, start: str, end: str) -> pd.DataFrame:
    ...
    if timeframe == "1m":
        ...
        if start_dt < limit_date:
            print(
                f"Warning: 1m data for yfinance is limited to last 30 days. Truncating start date from {start} to {limit_date.strftime('%Y-%m-%d')}"
            )
            start_dt = limit_date
        ...
        if start_dt < limit_date:
            print(
                f"Warning: Intraday data ({timeframe}) for yfinance is limited to last 60 days. Truncating start date from {start} to {limit_date.strftime('%Y-%m-%d')}"
            )
            start_dt = limit_date

    if start_dt >= end_dt:
        print(
            f"No new dates to query for {symbol} ({timeframe}) after yfinance limitation check."
        )
        return pd.DataFrame()
    ...
    # Download data
    print(
        f"Downloading yfinance data for {symbol} ({interval}) from {start_str} to {end_str}..."
    )
    df = yf.download(
        tickers=symbol,
        start=start_str,
...
def ingest_yfinance(
    ...
    recalculate_catalog_entry(
        symbol=symbol, timeframe=timeframe, asset_class=asset_class, source="yfinance"
    )

    print(
        f"Successfully ingested yfinance data for {symbol} ({timeframe}). Rows: {len(standardized)}"
    )
    return {
        "symbol": symbol,
        "timeframe": timeframe,
