...
def cross_validate(symbol: str, 
                   timeframe: str, 
                   start_date: str, 
                   end_date: str, 
                   source1: str = "alpaca", 
                   source2: str = "yfinance") -> dict:
    """
    ...
    df = get_ohlcv(symbol, timeframe=timeframe, start=start_date, end=end_date, adjusted=False)
    if len(df) == 0:
    ...
    # Merge on timestamp
    merged = pd.merge(df_s1, df_s2, on='timestamp', suffixes=(f'_{source1}', f'_{source2}'))
    ...
    if len(significant_diffs) > 0:
        print(f"Warning: Found {len(significant_diffs)} rows with >1% price difference.")
        print(significant_diffs[['timestamp', f'close_{source1}', f'close_{source2}']].head())
