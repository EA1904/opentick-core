...
from tvdata import cross_validate, get_ohlcv, ingest_alpaca, ingest_yfinance
from tvdata.config import DB_PATH, LAKE_ROOT

@pytest.mark.skipif(not os.environ.get("ALPACA_API_KEY"), reason="ALPACA_API_KEY is not set")
def test_alpaca_integration():
    ...
    df_alpaca = get_ohlcv(
        symbol, timeframe=timeframe, start=start_str, end=end_str, adjusted=False
    )
    ...
    report = cross_validate(
        symbol, timeframe, start_str, end_str, source1="alpaca", source2="yfinance"
    )
    ...
    # Check score
    assert report.get("reliability_score", 0.0) >= 70.0, (
        f"Reliability score is too low: {report.get('reliability_score')}"
    )
    print(
    ...
    if os.path.exists(db_path):
        ...
        conn.execute(
            f"DELETE FROM data_catalog WHERE symbol = '{symbol}' AND timeframe = '{timeframe}'"
        )
        conn.commit()
        conn.close()
    ...
    for year in set(years):
        partition_dir = os.path.join(
            lake_root, "asset_class=stocks", f"timeframe={timeframe}", f"year={year}"
        )
