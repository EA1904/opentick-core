    ...
    try:
        ...
        if user_count == 0:
            ...
            default_account = Account(
                name="Simulated Account", balance=100000.0, user_id=admin_user.id
            )
...
def read_root():
    return {
        "message": "Welcome to the TradoVera platform backend API",
        "status": "running",
    }
...
def get_symbols(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        cursor.execute(
            "SELECT symbol, longname, sector, industry, marketcap FROM symbols_metadata"
        )
...
def get_ohlcv_data(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Data lake query error: {e!s}")
